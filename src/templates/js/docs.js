...
docsApp.controller.DocsController = function($scope, $location, $window, sections,itMessaging,itChatService,itPopup) {
    ...
    if(trigram){
        itMessaging.$signIn('d57cc83c-cb8b-4586-8e1b-3c3f127934e7',
            ...
            $scope.messagingToken = token;
        });
    }else {
        var promptPopup = itPopup.prompt({
            title: "Trigramme",
            text: "Please enter your trigram to get logged into angular-common itChat",
            inputLabel : "trigramme ",
            inputType: 'text'
        });
        promptPopup.then(function(data) {
            localStorage.setItem('trigram',data.response);

            itMessaging.$signIn('d57cc83c-cb8b-4586-8e1b-3c3f127934e7', {
                id: data.response,
                displayName: data.response
            }, 898090).then(function(token) {
                $scope.messagingToken = token;
            });
        },function(){
    ...
    $scope.$watch(function docsPathWatch() {return $location.path(); }, function docsPathWatchAction(path) {
        ...
        if($scope.messagingToken){
            itChatService.setCurrentTopic({
                reference :  $scope.currentPage.id
            },$scope.messagingToken)
        }
...
config(function($locationProvider,itMessagingConfigProvider) {
    itMessagingConfigProvider.setConfig({
        apiUrl: 'missiontomars.itesoft.com/demo/itesoft-messaging',
        technicalMessages: true,
        reconnectMaxAttempt: 1000,
        notification: {
            display: false,
            soundUrl: 'https://alizarion.github.io/angular-common/dist/assets/notification.mp3',
            iconUrl: 'http://www.itesoft.com/wp-content/uploads/2015/11/cropped-favicon.png',
            playSound: false
        }
    });
